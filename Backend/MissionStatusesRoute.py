...
class MissionStatusesRoute(Resource):
   def post(self):
      try:
         # Parse the arguments.
         parser = reqparse.RequestParser()
         parser.add_argument('statusid')
         parser.add_argument('missionid')
         args = parser.parse_args()
         # Figure out a good id to use for our new item.
         max_id = Missionstatuses.select(fn.MAX(Missionstatuses.id)).scalar()
         args['id'] = max_id+1
         # Load 'em into a DB model.
         entry = Missionstatuses(**args)
   ...
   def put(self):
      try:
         ...
         entry = Missionstatuses.get_by_id(int(args['id']))
         entry.statusid = args['statusid']
